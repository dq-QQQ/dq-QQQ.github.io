...
# @ObservedObject, @StateObject, @State 차이점

---

-	누가 뷰모델을 소유하느냐, 무엇을 관찰하느냐에서 차이가 있음
-	@StateObject는 뷰가 뷰모델을 직접 생성하고 생명주기동안 책임짐
	-	뷰가 다시 그려져도 뷰모델은 새로 생성되지 않음
-	@ObservedObject는 소유하지 않고 관찰만함
	-	뷰가 다시 그려지면 뷰모델은 새로 생성됨
	-	아무도 영구적으로 소유하지 않음
-	@State는 소유하지만 @Published를 구독하지 않음
