...
-   기본적으로 @protected이지만 접근은 가능하다. @public으로 하는것도 안좋다.
-   인스턴스 변수는 getter와 setter가 필요함 따라서 @property라는 개념이 생김
-   프로퍼티를 @synthesize하면 getter와 setter를 자동으로 생성해줌 & 인스턴스 변수랑 연결도 가능
-   @dynamic을 사용하면 내가 getter&setter를 만들겠다는 소리 (기본값)
-   `readonly` : 읽기 전용
-   `readwrite` : 읽기 쓰기 가능 (기본값)
-   `assign` : 대입으로 값 설정 (기본값)
-   `retain` : MRC 카운팅 +1
-   `strong` : ARC 카운팅 +1
-   `weak` : ARC 카운팅 +0
-   `copy` : 객체 복사용
-   `nonatomic` :  게터와 세터를 여러 스레드가 동시에 실행될수도 있음 (기본값)